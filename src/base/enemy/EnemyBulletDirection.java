...
public class EnemyBulletDirection extends Renderer {
    BufferedImage image;
    AffineTransform transform;
    AffineTransform emptyTransform;
    double angle;

    public EnemyBulletDirection(BufferedImage image) {
        this.image=image;
        this.angle = 0;
    ...
    public void render(Graphics g, GameObject master) {
        EnemyBullet playerBullet = (EnemyBullet) master; // rotate
        Graphics2D g2D = (Graphics2D) g;
        this.syncTransform(playerBullet, this.image);
        g2D.drawImage(this.image, this.transform, null);
    }

    public void syncTransform(EnemyBullet master, BufferedImage image) {
