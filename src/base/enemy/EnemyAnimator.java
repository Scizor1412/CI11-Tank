...
public class EnemyAnimator extends Renderer {
    ...
    public EnemyAnimator (ArrayList<BufferedImage> images, int maxFramecount) {
        this.images = images;
    ...
    public void render(Graphics g, GameObject master) {
        Enemy enemy = (Enemy) master; // rotate
        Graphics2D g2D = (Graphics2D) g;

        BufferedImage currentImage = this.images.get(currentImageIndex);
        this.syncTransform(enemy, currentImage);
    ...
    public void syncTransform(Enemy master, BufferedImage image) {
